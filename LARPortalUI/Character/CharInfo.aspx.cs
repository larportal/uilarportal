...
namespace LarpPortal.Character
{
    public partial class CharInfo : System.Web.UI.Page
    {
        ...
        private void DisplayCharacter(Classes.cCharacter CharInfo)
        {
            ...
            //  Always want to fill this in since the screen takes care of whether to display it or not.
            tbTotalCP.Text = CharInfo.TotalCP.ToString();

            foreach (Classes.cSkillPool SkillPool in CharInfo.SkillPools)
            {
                if (SkillPool.DefaultPool)
                    lblPoolName.Text = SkillPool.PoolDescription;
            }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if ((oCharSelect.SkillSetID.HasValue) && (oCharSelect.CharacterInfo != null))
            {
                ...
                if (cChar.CharacterType == 2)           //  NPC
                {
                    double dTemp;
                    if (double.TryParse(tbTotalCP.Text, out dTemp))
                        cChar.TotalCP = dTemp;
                }
