...
namespace LarpPortal.Reports
{
    public partial class PELAnswerSummary : System.Web.UI.Page
    {
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=PELAnswerSummary.csv");
            Response.Charset = "";
            ...
            {
                columnbind.Append(gvAnswers.Columns[k].HeaderText + ',');
            }
            columnbind.Append("\r\n");
            for (int i = 0; i < gvAnswers.Rows.Count; i++)
            {
                for (int k = 0; k < gvAnswers.Columns.Count; k++)
                {
                    CellText = gvAnswers.Rows[i].Cells[k].Text;
