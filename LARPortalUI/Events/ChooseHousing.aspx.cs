...
{
    public partial class ChooseHousing : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["RegistrationID"] != null)
                    hidRegID.Value = Session["RegistrationID"].ToString();
                else
                    hidRegID.Value = "19471";
            }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            if ((!IsPostBack) || (_Reload))
            {
                ...
                sortedList.Add("@RegistrationID", hidRegID.Value);
                DataTable dtLocations = Classes.cUtilities.LoadDataTable("uspGetAvailBedsForEvent", sortedList, "LARPortal", Session["UserName"].ToString(), lsRoutineName);
                if (dtLocations.Columns["DisplayButton"] == null)
                    dtLocations.Columns.Add("DisplayButton", typeof(Boolean));
                int numAvail;
                bool bSitesAvail = false;
                foreach (DataRow dr in dtLocations.Rows)
                {
                    dr["DisplayButton"] = false;
                    if (int.TryParse(dr["BedsAvail"].ToString().Trim(), out numAvail))
                    {
                        bSitesAvail = true;
                        if (numAvail == 0)
                            dr["DisplayButton"] = false;
                        else
                            dr["DisplayButton"] = true;
                    }
                }

                if (bSitesAvail)
                {
                    gvHousingOptions.DataSource = dtLocations;
                    gvHousingOptions.DataBind();
                }
                else
                {
                    gvHousingOptions.DataSource = null;
                    gvHousingOptions.DataBind();
                }
        ...
        protected void gvHousingOptions_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            string sLocation = "";
            SortedList sortedList = new SortedList();
            ...
            {
                if (e.CommandArgument.ToString() == dataRow["LocationID"].ToString())
                    sLocation = dataRow["LocationName"].ToString();
            }
            sortedList.Add("@RegistrationID", hidRegID.Value);
            sortedList.Add("@AssignHousingID", e.CommandArgument);
            sortedList.Add("@AssignHousing", sLocation);
            Classes.cUtilities.PerformNonQuery("uspInsUpdCMRegistrations", sortedList, "LARPortal", Master.UserName);

            lblmodalMessage.Visible = true;
            lblmodalMessage.Text = "Your housing choice has been saved.";
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openMessage();", true);
        ...
        {
            Response.Redirect("/Events/EventPayment.aspx", true);
        }
    }
}