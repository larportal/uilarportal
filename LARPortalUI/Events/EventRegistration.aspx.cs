...
namespace LarpPortal.Events
{
    public partial class EventRegistration : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                hidPlayerRegistering.Value = Master.UserID.ToString();
            }
        }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            if ((!IsPostBack) || (_Reload))
            {
                ...
                ddlPlayerName.DataSource = dvPlayerList;
                ddlPlayerName.DataTextField = "PlayerFirstLastName";
                ddlPlayerName.DataValueField = "UserID";
        ...
        protected void NotifyOfNewRegistration(bool bNewRegistrion)
        {
            ...
            if (dtCampaignInfo.Rows.Count > 0)
            {
                ...
                //  JB  01/30/2022  If the user email is not equal to the player email, add it to the list to send to.
                string sUserEmail = "";
                ...
                strBody = lblPlayerName.Text + " has just ";

                if (sUserEmail.Length > 0)
                    strBody += "been ";

                strBody += "registered for the upcoming " + drCampInfo["CampaignName"].ToString() + " event.  <br>" +
        ...
        {
            hidLastPlayerRegister.Value = ddlPlayerName.SelectedValue;
            ddlEventDate_SelectedIndexChanged(null, null);
        ...
        protected void SetupPlayerChoice()
        {
                ...
                (Master.RoleString.Contains(Classes.cConstants.LOGISTICS_EVENT_REGISTRATION_APPROVAL_37)))
            {
                ...
                hidRegisterOthers.Value = "1";
            }
            else
            {
                lblPlayerName.Visible = true;
                ddlPlayerName.Visible = false;
                hidRegisterOthers.Value = "";
                hidLastPlayerRegister.Value = Master.UserID.ToString();
            }
        }
        ...
        protected void LoadPlayerList()
        {
            ...
            if (ddlPlayerName.SelectedIndex >= 0)
                ...
                hidLastPlayerRegister.Value = "";

            //  Since we can do everybody, go get the registration status for this event.
            DataTable dtPlayers = new DataTable();
            SortedList sPlayerParams = new SortedList();
            sPlayerParams.Add("@EventID", ddlEventDate.SelectedValue);
            dtPlayers = Classes.cUtilities.LoadDataTable("uspGetCampaignPlayersForEvent", sPlayerParams, "LARPortal", Master.UserName, lsRoutineName);
            DataView dvPlayerList = new DataView(dtPlayers, "", "PlayerNameAndStatus", DataViewRowState.CurrentRows);

            ddlPlayerName.DataSource = dvPlayerList;
            ddlPlayerName.DataTextField = "PlayerNameAndStatus";
            ddlPlayerName.DataValueField = "UserID";
            ddlPlayerName.DataBind();
            if (hidLastPlayerRegister.Value.Length > 0)
            {
                foreach (ListItem liPlayer in ddlPlayerName.Items)
                {
                    if ((liPlayer.Value == hidLastPlayerRegister.Value) &&
                        (liPlayer.Value != ""))
                    {
                        ddlPlayerName.ClearSelection();
                        liPlayer.Selected = true;
                    }
                }
            }
            if (ddlPlayerName.SelectedIndex < 0)
            {
                // Nothing select so select the first item.
                ddlPlayerName.ClearSelection();
